...
namespace rs2
{
    ...
    inline void aus_set_counter(const char* counter, int value = 0)
    {
        rs2_error* e = nullptr;
        rs2_aus_set_counter(counter, value, &e);
        error::handle(e);
    }
    ...
    inline std::vector<std::string> aus_get_timers_names()
    {
        rs2_error* e = nullptr;
        auto buffer = rs2_aus_get_timers_names(&e);
        std::shared_ptr<const rs2_aus_counters_names> list(buffer, rs2_aus_delete_counters_names);
        error::handle(e);
